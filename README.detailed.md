...
| **Tool hooks** | `gen_ai.operation.name` = `execute_tool`; span name `execute_tool <tool_name>` per [execute tool span](https://opentelemetry.io/docs/specs/semconv/gen-ai/gen-ai-spans/#execute-tool-span), except the built-in [**Skill**](https://code.claude.com/docs/en/tools-reference.md) tool: `execute_skill` or `execute_skill <resolved_name>`. Span **kind** `INTERNAL`; `gen_ai.tool.name`, `gen_ai.tool.type` (`skill` for `Skill`, `extension` for `mcp__*`, else `function`); optional `gen_ai.tool.call.id` from `tool_use_id` / `tool_call_id`; `error.type` on failures / non-zero exit. **Skills:** `claude.is_skill_invocation` and `claude.skill_name` when the tool is `Skill` and the name can be read from `tool_input`. If Claude adds a top-level `skill_name` on other tools (active skill context), `claude.active_skill` is set. |
